...
export async function GET(request: NextRequest) {
  ...
  const period = searchParams.get('period') || 'annual'; // annual or quarter
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch income statement", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}