...
const getCurrencyDisplayText = (statements: any[]) => {
  ...
  if (!reportedCurrency) return 'In $m';
...
export function HistoricalFinancials() {
  ...
  const exportToExcel = async (allData: { incomeData: any[], cashFlowData: any[], balanceSheetData: any[] }, filename: string) => {
    ...
    const addDataToWorksheet = (worksheet: ExcelJS.Worksheet, data: any[][]) => {
      data.forEach(row => {
      ...
      worksheet.getColumn(1).width = 35; // First column wider
      for (let i = 2; i <= numDataColumns + 1; i++) {
    ...
    summarySheet.getColumn(1).width = 60;
    
    // Add Income Statement sheet
    const incomeSheet = workbook.addWorksheet('Income Statement');
    addDataToWorksheet(incomeSheet, formatDataForExcel(allData.incomeData, "Income Statement"));
    
    // Add Cash Flow sheet
    const cashFlowSheet = workbook.addWorksheet('Cash Flow');
    addDataToWorksheet(cashFlowSheet, formatDataForExcel(allData.cashFlowData, "Cash Flow Statement"));
    
    // Add Balance Sheet
    const balanceSheet = workbook.addWorksheet('Balance Sheet');
    addDataToWorksheet(balanceSheet, formatDataForExcel(allData.balanceSheetData, "Balance Sheet"));
  ...
  const handleExport = async (format: 'csv' | 'excel') => {
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        // Export as Excel
        await exportToExcel({ incomeData, cashFlowData, balanceSheetData }, filename);
